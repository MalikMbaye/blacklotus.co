(function( $ ){
    ...
    [].forEach.call(df_cptfilters, function(ele, index){
        ...
        let observer = new IntersectionObserver(function (item) {
            iso.layout();
        });
    ...
    function fetch_request( grid_container, ele_class, term_id, selector, selected_tax, _request = 'loadmore', current_column , search_value = '' ) {
        ...
        let excluded_post_ids = [];
        if (_request === 'loadmore' && df_cpt_filter[ele_class].orderby === '5') {
            let existing_items = selector.querySelectorAll('.df-cpt-item');
            existing_items.forEach(function(item) {
                let post_id = item.getAttribute('id');
                if (post_id && post_id.startsWith('post-')) {
                    let id = post_id.replace('post-', '');
                    if (id && !isNaN(id)) {
                        excluded_post_ids.push(parseInt(id));
                    }
                }
            });
        }
        fetch(ajaxurl, {
            ...
            body: new URLSearchParams({
                ...
                excluded_post_ids: excluded_post_ids.length > 0 ? excluded_post_ids.join(',') : '',
            })
        })
